...
### Jira-ready notes for bug cases
- Bug 1: Case-insensitive email handling in auth flows. Repro: register with mixed-case email, then use a different casing for forgot-password/login/reset and observe failure.
- Bug 2: Missing HR-only authorization on leave quota update/reset endpoints. Repro: authenticate as a non-HR employee and call the quota update/reset routes directly.
- Bug 3: Leave-day mismatch validation is too permissive. Repro: submit a leave request where the submitted day count does not match the actual valid weekdays after excluding weekends/holidays and observe acceptance.
- Bug 4: Role-based access control gaps around leave approvals and alternate handling. Repro: use employee/alternate tokens to access actions reserved for HoD/HR and observe whether the API blocks them.